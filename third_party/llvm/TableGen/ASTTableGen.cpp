...
namespace tablegen {
llvm::StringRef HasProperties::getName() const {
...
namespace {
StringRef removeExpectedNodeNameSuffix(Record *node, StringRef suffix) {
...
std::string DeclNode::getClassName() const {
  return (Twine(getName()) + "Decl").str();
}

StringRef DeclNode::getId() const { return getName(); }

// Type nodes are all named ending in Type, just like the corresponding
// C++ class, and the ID just strips this suffix.
StringRef TypeNode::getClassName() const { return getName(); }

StringRef TypeNode::getId() const {
...
StringRef StmtNode::getClassName() const { return getName(); }

std::string StmtNode::getId() const {
...
void visitASTNodeRecursive(ASTNode node, ASTNode base, ChildMap const &map,
...
void visitHierarchy(RecordKeeper &records, StringRef nodeClassName,
                    ASTNodeHierarchyVisitor<ASTNode> visit) {
...
void visitASTNodeHierarchyImpl(RecordKeeper &records, StringRef nodeClassName,
                               ASTNodeHierarchyVisitor<ASTNode> visit) {
